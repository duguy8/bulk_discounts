...
RSpec.describe InvoiceItem, type: :model do
  ...
  describe "instance methods" do
    before :each do
      ...
      @discount4 = create(:discount, merchant_id: @merchant3.id, quantity_threshold: 10, percentage_discount: 10)
      @discount5 = create(:discount, merchant_id: @merchant3.id, quantity_threshold: 10, percentage_discount: 20)
      ...
      @invoice_item4 = create(:invoice_item, item_id: @item4.id, invoice_id: @invoice3.id, quantity: 13, unit_price: 100)
    end

    it "#discount_applied" do
      expect(@invoice_item1.discount_applied).to eq(@discount1)
      expect(@invoice_item4.discount_applied).to eq(@discount5)
